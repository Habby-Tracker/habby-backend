...
module.exports = class Goal {
  ...
  static async updateById(id, user_id, attrs) {
    ...
    const { rows } = await pool.query(
      `
      UPDATE goals 
      SET    goal_category_id = $3, goal_name=$4, default_habit_name=$5, status_id=$6, timePeriodCount=$7
      WHERE  id=$1 
      AND    user_id=$2
      RETURNING *;
    `,
  ...
  static async getAll(user_id) {
    const { rows } = await pool.query(
      `
      SELECT g.*, s.name as status, c.name as category
      FROM  goals g
      JOIN status s on s.id = g.status_id
      JOIN categories c on c.id = g.goal_category_id
      WHERE g.user_id = $1

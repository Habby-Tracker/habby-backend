...
const newHabit = {
  ...
  habitName: 'read',
  statusID: '1',
  dueDate: '2022-03-23T20:30:00.000Z',
  completedDate: '2022-03-24T20:30:00.000Z',
};
const newHabit2 = {
  goalID: '2',
  habitName: 'eat veggies',
  statusID: '1',
  dueDate: '2022-03-29 13:30:00',
  completedDate: '2022-03-30 13:30:00',
};
...
describe('/api/v1/items', () => {
  ...
  it('GET / returns all items associated with the authenticated User', async () => {
    ...
    const { body: user1Habit } = await agent
      .post('/api/v1/habits')
      .send(newHabit);
    ...
    const { body: user2Habit } = await agent2
      .post('/api/v1/habits')
      .send(newHabit2);

    const resp1 = await agent.get('/api/v1/habits');
    expect(resp1.status).toEqual(200);
    expect(resp1.body).toEqual([{ ...user1Habit, status: 'Active' }]);

    const resp2 = await agent2.get('/api/v1/habits');
    expect(resp2.status).toEqual(200);
    expect(resp2.body).toEqual([{ ...user2Habit, status: 'Active' }]);
  });
  ...
  it('GET /:id should get a habit', async () => {
    ...
    expect(got).toEqual({ ...habit, status: 'Active' });
  });

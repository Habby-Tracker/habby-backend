...
module.exports = class Goal {
  ...
  static async insert({ goalCategoryID, goalName, timePeriodID, habitTypeID, habitName, statusID, user_id }) {
    const { rows } = await pool.query(
      ...
      RETURNING *;
    `,
      [user_id, goalCategoryID, goalName, timePeriodID, habitTypeID, habitName, statusID]
    );
  ...
  static async updateById(id, user_id, attrs) {
    ...
    if (!goal) return null;
    const { goalCategoryID, goalName, habitName, statusID } = { ...goal, ...attrs };
    const { rows } = await pool.query(
      `
      UPDATE goals 
